  ...
  methods: {
    initLineChart() {
      ...
      this.data.forEach((item) => {
        category.push(item.商品名称);
        importData.push(item['进口额/万美元']);
        exportData.push(item['出口额/万美元']);
      });
      const option = {
    ...
    initPieChart() {
      ...
      const trafficWay = this.data.map((item) => {
        return {
          name: item.商品名称,
          value: Number(item['出口占比(%)']),
        };
      });
    ...
    getData() {
      request('/trade/tradeAmountCount').then((res) => {
        console.log(res);
        this.data = res.data;
        this.initLineChart();
        this.initPieChart();
